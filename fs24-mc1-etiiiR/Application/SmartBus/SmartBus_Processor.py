...
class SmartBusProcessor:
    def __init__(self, servers):
        self.consumer = Consumer({
            ...
            'group.id': 'bus_process' + str(datetime.now().timestamp()),
    ...
    def process_bus_data(self, topic="bus_data"):
        ...
        while True:
            ...
            print(f"Bus {bus_id} has a service probability of {service_probability}")
            # If service probability is 80% or more, send bus into service and reset probabilities and seconds without service
            if service_probability is not None and service_probability >= 0.5:
...
class BusServiceResetter:
    ...
    def reset_bus_service(self, bus_id, topic="bus_service_station"):
        ...
        reset_message.timestamp = datetime.now().timestamp()

        producer_instance = Producer({'bootstrap.servers': servers})  # Replace 'your_server' with your actual server
        producer_instance.produce(topic, key=bus_id, value=reset_message.SerializeToString())
